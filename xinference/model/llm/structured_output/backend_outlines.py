...
from xinference.model.llm.structured_output.backend_types import (StructuredOutputBackend,
                                                     StructuredOutputGrammar,
                                                     StructuredOutputOptions)
from xinference.model.llm.structured_output.utils import (OutlinesVocabulary,
                                             get_outlines_cache,
...
class OutlinesBackend(StructuredOutputBackend):
    ...
    def _validate_tokenizer(self):
        ...
        for token in test_tokens:
            try:
                ...
                if token not in decoded and len(encoded) == 1:
                    logger.warning(f"Tokenizer inconsistency for '{token}': encoded={encoded}, decoded='{decoded}'")
            except Exception as e:
                logger.warning(f"Tokenizer validation failed for '{token}': {e}")

        logger.info(f"Tokenizer validation completed: vocab_size={self.tokenizer.vocab_size}")
    ...
    def allocate_token_bitmask(self, max_num_seqs: int, vocab_size: int = None) -> torch.Tensor:
        # Use provided vocab_size or fall back to self.vocab_size
        actual_vocab_size = vocab_size if vocab_size is not None else self.vocab_size
        return torch.full(
            (max_num_seqs, (actual_vocab_size + 31) // 32),
            -1,
            dtype=torch.int32,
